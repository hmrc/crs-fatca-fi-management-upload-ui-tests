...
trait BasePage extends BrowserDriver with Matchers with PageObject with IdGenerators {
  ...
  val baseUrlForFileUpload: String       = TestConfiguration.url("crs-fatca-reporting-frontend") + "/report"
  val baseUrlForManualSubmission: String = TestConfiguration.url("crs-fatca-manual-submission-frontend")
